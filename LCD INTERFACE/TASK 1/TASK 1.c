...
void init(void);//initialize cmds are there
void lcd_command(unsigned char);//used to send cmds to the lcd
void lcd_data(unsigned char);//used to send the data to the lcd

unsigned char arr[]={"HELLO WORLD"};//array named arr to store value
unsigned char i;

void main() {
    init();//calling the init() dunction
    lcd_command(0x80);//Taking the first row and first column of lcd
    for(i=0;i<11;i++){
